...
processed.cleveland.bin <- cbind(age = ((processed.cleveland$age - min(processed.cleveland$age)) / (max(processed.cleveland$age) - min(processed.cleveland$age))), as.data.frame(cbind(with(processed.cleveland, model.matrix(~ sex + 0)))), as.data.frame(cbind(with(processed.cleveland, model.matrix(~ cp + 0)))), trestps = ((processed.cleveland$trestbps - min(processed.cleveland$trestbps)) / (max(processed.cleveland$trestbps) - min(processed.cleveland$trestbps))),  chol = processed.cleveland$chol, as.data.frame(cbind(with(processed.cleveland, model.matrix(~ fbs + 0)))), as.data.frame(cbind(with(processed.cleveland, model.matrix(~ restecg + 0)))), thalach = ((processed.cleveland$thalach - min(processed.cleveland$thalach)) / (max(processed.cleveland$thalach) - min(processed.cleveland$thalach))), as.data.frame(cbind(with(processed.cleveland, model.matrix(~ exang + 0)))), oldpeak = ((processed.cleveland$oldpeak - min(processed.cleveland$oldpeak)) / (max(processed.cleveland$oldpeak) - min(processed.cleveland$oldpeak))), as.data.frame(cbind(with(processed.cleveland, model.matrix(~ slope + 0)))), as.data.frame(cbind(with(processed.cleveland, model.matrix(~ ca + 0)))), as.data.frame(cbind(with(processed.cleveland, model.matrix(~ thal + 0)))), as.data.frame(cbind(with(processed.cleveland, model.matrix(~ num + 0)))));
...
# Dividir dataset para obtener conjunto de entrenamiento 
set.seed(1234)
samples=sample(nrow(processed.cleveland), 0.7*nrow(processed.cleveland))
training.set = processed.cleveland[samples, ] # Se obtiene el 70% del dataset para entrenar el modelo
test.set = processed.cleveland[-samples, ] # Se obtiene el 30% del dataset para probar el modelo

# Se obtiene el clasificador bayesiano segun la variable num y utilizando el dataset para entrenamiento
bayesian.classifier = naiveBayes(num ~ ., data = training.set)
bayesian.classifier

# Luego se utiliza una matriz de confusion junto con el resto del dataset para probar el modelo
bayesian.results = predict(object = bayesian.classifier, test.set)
test.set["predicted"] = bayesian.results
conf.matrix.cb = confusionMatrix(table(test.set$num, test.set$predicted))
